...
| `navigate_to_feature` | Rust 直接 (macOS AX) | ヘルプメニューにキーワードを入力し、対応するメニュー項目をハイライトして機能の場所を示す |
...
プロジェクト指示（システムプロンプト）にキーワード対応表として記述する。
Claude がユーザーの質問から機能名を特定し `navigate_to_feature` を呼ぶ。

```
ユーザー: 「レイアウトを管理したい」
  ↓ Claude がプロジェクト指示の対応表を参照
  ↓ keyword = "レイアウトの管理" と判断
  ↓ navigate_to_feature { keyword: "レイアウトの管理" } を呼び出す
  ↓ FileMaker のヘルプ検索にキーワードが入力され、メニューがハイライト
```
...
1. ユーザーの意図から機能名を特定する
2. 下記のキーワード対応表から最も近い keyword を選ぶ
3. navigate_to_feature ツールを呼び出す
...
| ユーザーが言いそうなこと           | keyword              |
|------------------------------------|----------------------|
| レイアウトを管理したい、レイアウト一覧 | レイアウトの管理    |
| レイアウトを作りたい、新規レイアウト  | 新しいレイアウト    |
| フィールドを作りたい、フィールド定義  | フィールドの定義    |
| スクリプトを書きたい、スクリプト作成  | スクリプトワークスペース |
| リレーションを設定したい             | リレーションシップ  |
| 値一覧を作りたい                    | 値一覧の定義        |
| アカウントを管理したい、パスワード    | アカウントの管理    |
| 特権セットを変えたい                 | 特権セット          |
| ファイルを共有したい、ホスト          | ファイルの共有      |
| 印刷したい、印刷設定                 | 印刷の設定          |

対応表にない機能はユーザーの言葉をそのまま keyword に使ってください。
```

---

### テスト用プロンプト（スキルなしで直接呼ぶ場合）

```
navigate_to_feature ツールで keyword="レイアウトの管理" を実行して
```
